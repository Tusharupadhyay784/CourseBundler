...
export const login = catchAsyncError(async (req, res, next) => {
    ...
    if (!isMatch) return next(new ErrorHandler("Incorrect Password and Email", 401))
...
export const addToPlaylist = catchAsyncError(async (req, res, next) => {
    ...
    if (itemExist) return next(new ErrorHandler("Item Already Added", 409))
    user.playlist.push({
        course: course._id,
...
export const updateUser = catchAsyncError(async (req, res, next) => {
    ...
    if (users.role === "user") user.role = "admin";
...
export const deleteUser = catchAsyncError(async (req, res, next) => {
    const users = await User.findById(req.params.id);
    if (!users) return next(new ErrorHandler("User not Found", 404));
    await cloudinary.v2.uploader.destroy(users.avatar.public_id);
    //    cancel Subscription
    await users.remove();
